function [w0, QVU, stat, feasible, t_opt] = optim(model, data, rep, QVU, solver, lbw, ubw, lbg, ubg, t_opt, w0, feasible, stat, type, varargin)
...
if nargin > 15
    ...
    w0(:,rep) = GenerateW0(model,data,0);
elseif nargin == 15
    w0(:,rep) = varargin{1};
else
    w0(:,rep) = GenerateW0(model,data,rep, QVU);
    if rep > 1
...
if ~isfield(data, 'Duration'), t_opt(rep) = w_opt(1); end
